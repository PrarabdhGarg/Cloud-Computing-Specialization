...
if [ $joincount -eq 100 ]; then
	...
	cnt=0
	for i in $joinfrom
	do		jointo=`grep joined dbg.log | grep '^ '$i | cut -d" " -f4-7 | grep -v $i | sort -u | wc -l`
		if [ $jointo -eq 9 ]; then
			cnt=`expr $cnt + 1`
		fi
	done
...
cnt=0
for i in $failednode
do	failcount=`grep removed dbg.log | sort -u | grep $i | wc -l`
...
tmp=0
for i in $failednode
do	accuracycount=`grep removed dbg.log | sort -u | grep -v $i | wc -l`
